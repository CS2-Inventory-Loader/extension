...
import { getRuntime, isSameOrigin } from '~/lib/messaging/utils';

import type { MessageRequest } from '~/lib/messaging/types';

export function relayMessages(onMessage = (req: MessageRequest) => getRuntime().sendMessage(null, req), messagePort = globalThis.window) {
...
function messageListener(req: MessageRequest, _sender: chrome.runtime.MessageSender, sendResponse: (response) => void) {
